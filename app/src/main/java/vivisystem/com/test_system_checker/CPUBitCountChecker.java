 ...
 */
public class CPUBitCountChecker {
...
//    [java] view plain copy 在CODE上查看代码片派生到我的代码片
    ...
    public static String getArchType(Context context) {
        if (getSystemProperty(CPU_ARCHITECTURE_KEY_64, "").length() > 0) {
            if (LOGENABLE) {
                Log.d("Hughie","CPU arch is 64bit");
            }
            ...
            if (LOGENABLE) {
                Log.d("Hughie","return cpu DEFAULT 32bit!");
            }
        ...
        if (cpuInfo != null && cpuInfo.exists()) {
            ...
            try {
                ...
                if (line != null && line.length() > 0 && line.toLowerCase(Locale.US).contains("arch64")) {
                    if (LOGENABLE) {
                        Log.d("Hughie", PROC_CPU_INFO_PATH + " contains is arch64");
                    }
                    return true;
                } else {
                    if (LOGENABLE) {
                        Log.d("Hughie", PROC_CPU_INFO_PATH + " is not arch64");
                    }
                }
            } catch (Throwable t) {
                if (LOGENABLE) {
                    Log.d("Hughie","read " + PROC_CPU_INFO_PATH + " error = " + t.toString());
                }
            } finally {
                try {
    ...
    private static boolean isLibc64() {
        ...
        if (libcFile != null && libcFile.exists()) {
            ...
            if (header != null && header[EI_CLASS] == ELFCLASS64) {
                if (LOGENABLE) {
                    Log.d("Hughie", SYSTEM_LIB_C_PATH + " is 64bit");
                }
                return true;
            }
        }
        ...
        if (libcFile64 != null && libcFile64.exists()) {
            ...
            if (header != null && header[EI_CLASS] == ELFCLASS64) {
                if (LOGENABLE) {
                    Log.d("Hughie", SYSTEM_LIB_C_PATH_64 + " is 64bit");
                }
                return true;
            }
        }
            ...
            try {
                ...
                if (inputStream != null) {
                    ...
                    if (count == 16) {
                        ...
                        if (LOGENABLE) {
                            Log.e("Hughie", "Error: e_indent lenght should be 16, but actual is " +  count);
                        }
                    }
                }
            } catch (Throwable t) {
                if (LOGENABLE) {
                    Log.e("Hughie", "Error:" + t.toString());
                }
            } finally {
                if (inputStream != null) {
                    try {
